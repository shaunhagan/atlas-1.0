"""
ATLAS AI
Stock Paper Trading Engine

Mirrors paper_trader.py exactly, using stock_portfolio/stock_logger
and the STOCK_* config constants, so stocks and crypto trade with
fully independent capital, risk parameters, and history.
"""
...
from stock_logger import log_trade
...
POSITION_ALLOCATION = 1.0 / STOCK_MAX_OPEN_TRADES
...
def _open_position(symbol, price, confidence, atr):
    ...
    starting balance (STOCK_RISK_PER_TRADE) on the ATR-derived stop
    ...
    if len(positions) >= STOCK_MAX_OPEN_TRADES:
        return False
    ...
    fill_price = price * (1 + STOCK_TRADING_FRICTION_PCT)

    stop_distance = STOCK_ATR_STOP_MULTIPLIER * atr
    ...
    take_profit = fill_price + (
        STOCK_RISK_REWARD_RATIO * stop_distance
    )

    risk_amount = starting_balance * STOCK_RISK_PER_TRADE
    ...
    print()
    print(
        f"STOCK PAPER BUY : {symbol} @ {price:.6f} "
