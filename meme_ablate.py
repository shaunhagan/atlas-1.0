...
=========================================
ATLAS AI
Meme Coin Signal Component Ablation
=========================================

Mirrors ablate.py/stock_ablate.py: does each individual scoring
component in signals.py actually contribute positive out-of-sample
expectancy for meme coins specifically, or is one dead weight (or
...
ABLATION_SYMBOL_LIMIT = meme_backtest.SYMBOL_LIMIT
...
def run_ablation(symbol_limit=ABLATION_SYMBOL_LIMIT):
    ...
    all_symbols = meme_backtest.exchange.get_markets()[:symbol_limit]
    ...
    for symbol in all_symbols:
        candle_cache[symbol] = meme_backtest.fetch_history(symbol, days=ABLATION_DAYS)
    ...
    print()
    print("=" * 100)
    print("MEME COIN SIGNAL COMPONENT ABLATION")
    print("=" * 100)
    print(tabulate(
        rows,
        headers=[
